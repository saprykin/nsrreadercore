...
FoFiType1C::~FoFiType1C() {
  ...
  if (encoding &&
      (const char **) encoding != fofiType1StandardEncoding &&
      (const char **) encoding != fofiType1ExpertEncoding) {
    for (i = 0; i < 256; ++i) {
      gfree(encoding[i]);
    }
    gfree(encoding);
...
void FoFiType1C::convertToType1(char *psName, const char **newEncoding, GBool ascii,
  ...
  if (!newEncoding && (const char **) encoding == fofiType1StandardEncoding) {
